<template>
    <div class="relative" ref="container">
        <div class="text-xs text-gray-400 mb-1.5">
            Módulo <span class="text-gray-500 italic">(Camada do sistema / Contexto / Área)</span>
        ...
        <div v-if="selected && !isOpen"
            ...
            @click="isOpen = true">
            <span class="inline-block w-2 h-2 rounded-full shrink-0" :style="{ backgroundColor: selected.color }" />
            <span class="truncate">{{ selected.name }}</span>
            <button type="button" @click.stop="clear" class="shrink-0 hover:opacity-70 ml-auto">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
        </div>

        <div v-else class="relative">
            <input ref="input" v-model="query" type="text" placeholder="Buscar módulo..."
        ...
        <div v-if="isOpen"
            ...
            <button v-for="(mod, index) in filtered" :key="mod.id" type="button"
                @mousedown.prevent="select(mod)"
                class="w-full text-left px-3 py-2 text-sm transition-colors"
                :class="index === highlightedIndex
                    ? 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white'
                    : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800'">
                <span class="inline-block w-2 h-2 rounded-full mr-2 shrink-0" :style="{ backgroundColor: mod.color }" />
                {{ mod.name }}
            </button>
...
const selected = computed(() => {
    ...
    return props.modules.find(m => m.id === props.modelValue) || null
})
...
const exactMatch = computed(() => {
    const q = query.value.toLowerCase().trim()
    if (!q) return false
    return props.modules.some(m => m.name.toLowerCase() === q)
})
